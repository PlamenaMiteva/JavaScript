function solve(input) {
    ...
    trainers.sort(function(a, b) {
        ...
        if (parseInt(a.Quantity) < parseInt(b.Quantity)) {
            ...
            if (parseInt(a.lecturesPerDay) > parseInt(b.lecturesPerDay)) {
                return 1;
            } if (parseInt(a.lecturesPerDay) < parseInt(b.lecturesPerDay)) {
                return -1;
            }
    ...
    if (sortCriteria === 'level') {
        students.sort(function(a, b) {
            var firstlevel=parseInt(a.level);
            var seclevel=parseInt(b.level);
                var first = parseInt(a.id);
                var sec = parseInt(b.id);
                if (Number(firstlevel) > Number(seclevel)) {
                    return 1;
                };
                if (Number(firstlevel) < Number(seclevel)) {
                    return -1;
    ...
    students.sort(function(a, b) {
        ...
        var strg = "" + (b.firstname).toString();
        var last = "" + (a.lastname).toString();
        var lastn = "" + (b.lastname).toString();
        if (str > strg) {
            return 1;
        }
        if (str < strg) {
...
for (var i in students) {
    ...
    delete students[i].town;
    delete students[i].role;
    delete students[i].level;
    delete students[i].certificate;
    var sum = 0;
    var counter = 0;
    ...
    var avg = sum / counter;
    delete obj.grades;
    obj.averageGrade = avg.toFixed(2);
    obj.certificate = cert;
};
for (var j in trainers) {
    delete trainers[j].town;
    delete trainers[j].role;
    delete trainers[j].Quantity;
...
console.log(solve(['name^courses',
